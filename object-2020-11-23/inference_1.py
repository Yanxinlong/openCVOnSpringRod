...
ser_1 = serial.Serial('COM10', 9600, timeout=2)                         # 设置一个串口对象(继电器),当给定端口时，该端口在对象创建时立即打开。
...
#=====释放驱动图像缓存
        ...
        with torch.no_grad():
            record = detect_1(model,cvImage,image_tensor)                   # 数据接口,record是一个整型列表，其中0=trip,1=no_trip

            if 1 in record:                           
                light_state = 1                                                    # 检测到异常，报警灯状态置1，报警灯常亮，直到遇到正常情况
                alarm_1(light_state)
                print("Camera_1 record:",record)
                print("Light_1 state:",light_state)
                save_normal(cvImage,camera_id = 1)                                 # 将文件命名为时间，保存异常情况时的场景
