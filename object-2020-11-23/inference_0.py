...
import torch
import serial
from tools import save_normal

g_cameraStatusUserInfo = b"statusInfo"

ser_0 = serial.Serial('COM8', 9600, timeout=2)                          # 设置一个串口对象(继电器),当给定端口时，该端口在对象创建时立即打开。
...
# get_infer_image用来获取推理能使用的图像数据
def get_infer_image(image,img_size):
    # 输入是(1024,1280,3)的numpy数组
    image = cv2.resize(image,(512,512))                             # (416,512,3)    
    image = image[:, :, ::-1].transpose(2, 0, 1)                    # (3, 416, 512)       
    image = np.ascontiguousarray(image)                             # (3, 416, 512)
    image = torch.from_numpy(image).to(device)                      # torch.Size([3, 416, 512])               
    image = image.float()  
    image /= 255.0  
    image = image.unsqueeze(0)                                      # torch.Size([1, 3, 416, 512])
...
def object_detect_0(model,img_size):    
#=====发现相机
...
#=====释放驱动图像缓存
        ...
        with torch.no_grad():
            record = detect_0(model,cvImage,image_tensor)                   # 数据接口,record是一个整型列表，其中0=trip,1=no_trip

            if 1 in record:                           
                light_state = 1                                                    # 检测到异常，报警灯状态置1，报警灯常亮，直到遇到正常情况
                alarm_0(light_state)
                print("Camera_0 record:",record)
                print("Light_0 state:",light_state)
                save_normal(cvImage,camera_id = 0)                                 # 将文件命名为时间，保存异常情况时的场景
        ...
        print("Camera_0 FPS:",1/(end-start))
...
def main_0():
    model =  get_model_0(cfg, img_size,weights,device)  
    state = object_detect_0(model,img_size)                                   # 状态，0-正常，-1-其他问题
