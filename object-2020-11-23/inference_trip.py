...
def detect_trip(model,img_size):    
#=====发现相机
...
#=====释放驱动图像缓存
        ...
        with torch.no_grad():
            record = detect(model,cvImage,image_tensor)                          # 数据接口,record是一个整型列表，其中0=W型,1=三型
            if record:                                                      
                trip_classes.append(record[0])

        if len(trip_classes)>=5:
            return trip_classes[-1]
...
def get_trip_class():
    model =  get_model_class(cfg, img_size,weights,device)  
    trip_class = detect_trip(model,img_size)                                   # 整型0/1
