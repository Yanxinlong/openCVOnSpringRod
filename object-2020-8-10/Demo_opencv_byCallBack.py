...
# 取流回调函数Ex
def onGetFrameEx(frame, userInfo):                 #  frame: MVSDK.LP_GENICAM_Frame object
    if (g_isStop == 1):
        return
    ...
    print("BlockId = %d userInfo = %s"  %(frame.contents.getBlockId(frame), c_char_p(userInfo).value))   # BlockId = 25 userInfo = b'test'
...
#===================图像格式是 Mono8================================
    ...
    if imageParams.pixelForamt == EPixelType.gvspPixelMono8:
        print("============")
        grayByteArray = bytearray(userBuff)                                                    # x01\x02\x02\x02\x01\x01\
        cvImage = numpy.array(grayByteArray).reshape(imageParams.height, imageParams.width)    # ndarray ; (1024,1280)--(H,W) ; 可保存为图像,可直接可视化        
    else:
        # 转码 => BGR24
        rgbSize = c_int()
...
def demo():    
#====发现相机
    cameraCnt, cameraList = enumCameras()                                # cameraCnt = 1 ; cameraList = <MVSDK.LP_GENICAM_Camera object at 0x000001ADEB6A69C8>
...
#====显示相机信息
    for index in range(0, cameraCnt):
        ...
        print("\ncamera",camera)                                          # <MVSDK.GENICAM_Camera object at 0x0000021AE16D6A48>
        print("\nCamera Id = " + str(index))                              
...
#====创建流对象
    streamSourceInfo = GENICAM_StreamSourceInfo()
    print(dir(streamSourceInfo))                                         # 查看流对象属性
    streamSourceInfo.channelId = 0
    streamSourceInfo.pCamera = pointer(camera)
