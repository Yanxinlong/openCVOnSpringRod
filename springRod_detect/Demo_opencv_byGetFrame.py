...
#=====释放驱动图像缓存
        ...
        if imageParams.pixelForamt == EPixelType.gvspPixelMono8:
            ...
            cvImage = numpy.array(grayByteArray).reshape(imageParams.height, imageParams.width)  # (1024, 1280)或(480, 640, 3)  （W,H,3)
            # cv2.imwrite('./image/test.png',cvImage) 
        else:
            # 转码 => BGR24
            rgbSize = c_int()
        ...
        # print(cvImage.shape)
        # image=cv2.imread('./image/test.png',1)                                                    # (1024, 1280, 3)
