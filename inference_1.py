...
def object_detect_1(model,img_size):    
#=====发现相机
...
#=====释放驱动图像缓存
        ...
        with torch.no_grad():
            record = detect_1(model,cvImage,image_tensor)                   # 数据接口,record是一个整型列表，其中0=trip,1=bolt,2=no_trip,3=no_bolt
            
            # if (2 in record) or (3 in record):                            # 0/1--正常，2/3--异常
        ...
        print("Camera_1 FPS:",1/(end-start))
...
def main_1():
    model =  get_model_1(cfg, img_size,weights,device)  
    state = object_detect_1(model,img_size)                                   # 状态，0-正常，-1-其他问题
    ser.close()
